...
import type { PaperStatus } from '@/components/papers/types';

const allowedStatuses: PaperStatus[] = ['pending', 'in_progress', 'completed'];
...
export async function POST(req: NextRequest) {
  ...
  if (!doi || !status) {
    return NextResponse.json({ message: 'Paper DOI and status are required.' }, { status: 400 });
  }
  ...
  const result = db
    .prepare('UPDATE user_paper_todo SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE user_id = ? AND doi = ?')
    .run(status, sessionUserId, doi);
